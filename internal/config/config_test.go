...
func TestLoad_MissingEncryptionKey(t *testing.T) {
	...
	_, err := Load()
	if err == nil {
		t.Fatal("expected error when APP_ENCRYPTION_KEY is missing")
	}
	if !strings.Contains(err.Error(), "APP_ENCRYPTION_KEY is required") {
		t.Errorf("error = %q, want it to mention APP_ENCRYPTION_KEY", err)
	}
}
